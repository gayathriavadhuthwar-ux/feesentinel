...
Copy-Item "$PSScriptRoot\..\Dockerfile" "$StagingDir\"
Copy-Item "$PSScriptRoot\..\render.yaml" "$StagingDir\"
Copy-Item "$PSScriptRoot\..\requirements.txt" "$StagingDir\"
Copy-Item "$PSScriptRoot\..\Procfile" "$StagingDir\" -ErrorAction SilentlyContinue

# Copy feemanagement folder content
& robocopy "$PSScriptRoot" "$StagingDir\feemanagement" /S /XF "*.zip" "*.pyc" ".env" "db.sqlite3" "package_project.ps1" /XD ".git" "venv" ".venv" "__pycache__" ".gemini" "brain" "staticfiles" ".agents" ".agent" | Out-Null
